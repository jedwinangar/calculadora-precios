...
/// exp(x)  = e elevado a x
///
/// Las funciones no distinguen mayúsculas/minúsculas:
/// ln(C), LN(C), sqrt(A), etc.
class FormulaEvaluator {
  ...
  double _parsePrimary() {
    ...
    if (identifier != null) {
      ...
      // Funciones matemáticas: ln(C), sqrt(A), abs(P), exp(C), etc.
      if (_match('(')) {
        ...
        if (!_match(')')) {
          throw const FormatException(
            'Falta un paréntesis de cierre en la función.',
          );
        }
      ...
      final variable = identifier.toUpperCase();
      if (variable == 'C' || variable == 'A' || variable == 'P') {
      ...
      throw FormatException(
        'Función o variable "$identifier" no reconocida.',
      );
    }
  ...
  double _applyFunction(String name, double argument) {
    switch (name.toLowerCase()) {
      case 'ln':
        if (argument <= 0) {
          throw const FormatException(
            'ln() solo está definido para valores mayores que cero.',
          );
        }
      ...
      case 'sqrt':
        if (argument < 0) {
          throw const FormatException(
            'sqrt() no acepta valores negativos.',
          );
        }
          ...
          Card(
            child: Padding(
              ...
              child: const Text(
                ...
                'Funciones: ln(x), sqrt(x), abs(x) y exp(x).',
              ),
            ),
          ),
